...
def pdf_to_txt_3(pdf_path, extract_image=False):
                ...
                if is_element_inside_any_table(element, page ,tables):
                    ...
                    if table_found == table_in_page and table_found != None:
                        ...
                        data['data'].append({
                                'page_num': pagenum+1,
                                'type': 'table',
                                'content': text_from_tables[table_in_page],
                            })
                        table_in_page+=1
...
def pdf_to_txt_pypdf2(pdf_path):
    """
...
def save_failed_pdfs(failed_pdfs, failed_dir):
    ...
    with open(os.path.join(failed_dir, 'failed_pdfs.txt'), 'w') as f:
...
if __name__ == '__main__':
    ...
    failed_pdfs = []  # List to store the names of failed PDFs

    # Process each PDF file
    # for pdf_file in get_input_pdfs(input_dir):
    for pdf_file in tqdm(get_input_pdfs(input_dir)):
        # print(os.path.join(input_dir, pdf_file))
        json_file_name = pdf_file[:-4] + '.json'
        json_file_path = os.path.join(cache_dir, json_file_name)
    ...
    save_failed_pdfs(failed_pdfs, failed_dir)
